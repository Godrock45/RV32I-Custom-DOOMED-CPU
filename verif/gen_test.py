...
# PAD nops are inserted after every instruction. PAD=4 fully separates instructions
# so nothing overlaps in a 5-stage pipeline -- a correct pipeline then produces
# exactly the same results as a single-cycle core, and these same assertions apply.
# PAD=0 is the real test.  Override with:  PAD=4 python gen_test.py
...
def e(instr, note=""):
    P.append((instr, note))
    here = len(P)-1
    for _ in range(PAD): P.append((0x00000013, ""))   # addi x0,x0,0
    return here
def idx(): return len(P)

# bytes to advance n *logical* instructions (padding-aware branch/jump offsets)
def slots(n): return n * (1 + PAD) * 4
...
def taken(mk):        # MUST take: +8 skips the poison
    e(mk(slots(2))); e(addi("x28","x28",1), "poison (branch should skip)")
def nottaken(mk):     # must NOT take: +8 would skip the counter, which we'd detect
    global _nt; _nt += 1
    e(mk(slots(2))); e(addi("x9","x9",1), "must execute"); e(nop())
...
# ---------------- phase 5: JAL / JALR ----------------
jal_i = idx()
e(jal("x29",slots(2)));                 EXP[29] = (jal_i*4 + 4) & M32   # link
...
jalr_i = idx()
e(jalr("x30", (jalr_i - jalr_setup)*4 + slots(2), "x5")); EXP[30] = (jalr_i*4 + 4) & M32
e(addi("x28","x28",1), "poison")                                 # must be skipped
...
jm_i = idx()
e(jalr("x7",slots(3)+1,"x6"), "target has bit0 set -> must be masked")
EXP[7] = (jm_i*4 + 4) & M32
...
# ---------------- emit ----------------
# ROM must hold the padded program; index width follows the depth.
DEPTH = 1 << max(8, (len(P)-1).bit_length())
IDX_HI = DEPTH.bit_length() + 0   # log2(DEPTH)+1 == top bit of PC[hi:2]

with open("ROM_test.sv","w") as f:
    f.write("// generated by gen_test.py -- do not edit\n")
    f.write("module ROME(\n    input [31:0]PC,\n    output [31:0] IR\n);\n")
    f.write(f"reg [31:0] registan [{DEPTH-1}:0];\ninitial begin\n")
    for i,(w,note) in enumerate(P):
        f.write(f"    registan[{i}]=32'h{w:08X};" + (f"  // {note}\n" if note else "\n"))
    f.write(f"end\nassign IR=registan[PC[{IDX_HI}:2]];\nendmodule\n")
